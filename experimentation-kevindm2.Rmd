...
```{r}
best_exhaustive_log_model = lm(log(SalePrice) ~ MSSubClass + OverallQual + OverallCond + YearBuilt + YearRemodAdd  + MasVnrArea + BsmtFinSF1 + GrLivArea + GarageCars + PoolArea + PoolQC, no_na_data)
calc_loocv_rmse(best_exhaustive_log_model)
pred = predict(best_exhaustive_log_model, newdata = no_na_data)
actual = no_na_data$SalePrice
mape(actual, exp(pred))

```

```{r}
par(mfrow = c(1,2))
plot(fitted(best_exhaustive_model), resid(best_exhaustive_model))
abline(0,0)
plot(fitted(best_exhaustive_log_model), resid(best_exhaustive_log_model))
abline(0,0)
```

```{r}
library(lmtest)
bptest(best_exhaustive_model)
bptest(best_exhaustive_log_model)
```

```{r}
par(mfrow = c(1,2))
qqnorm(resid(best_exhaustive_model))
qqline(resid(best_exhaustive_model))
qqnorm(resid(best_exhaustive_log_model))
qqline(resid(best_exhaustive_log_model))
```

```{r}
shapiro.test(resid(best_exhaustive_model))
shapiro.test(resid(best_exhaustive_log_model))
```
